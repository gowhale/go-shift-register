...
func (m *mainTest) Test_getCoverage_noTestLine() {
	tl, err := getCoverage("?       go-shift-register/cmd/fruit  [no test files]")
...
func (m *mainTest) Test_getCoverage_excludedFile() {
	tl, err := getCoverage("?       go-shift-register  [no test files]")
...
func (m *mainTest) Test_covertOutputToCoverage_noTests_Pass() {
	commandOutput := `?       go-shift-register  [no test files]
	`
...
func (m *mainTest) Test_covertOutputToCoverage_testsLine_Pass() {
	commandOutput := `ok      go-shift-register/cmd/pkg-test      0.179s  coverage: 79% of statements
	`
	expectedTl := []testLine{{pkgName: "go-shift-register/cmd/pkg-test", coverage: underCoverage, coverLine: true}}
...
func (m *mainTest) Test_covertOutputToCoverage_testsLine_Fail() {
	commandOutput := `ok      go-shift-register/cmd/pkg-test      0.179s  coverage: wrong% of statements
	`
...
func (m *mainTest) Test_validateTestOutput_sufficentCov_testLine() {
	expectedTl := []testLine{{pkgName: "go-shift-register/cmd/pkg-pizza", coverage: overCoverage, coverLine: true}}
...
func (m *mainTest) Test_validateTestOutput_NOT_sufficentCov_testLine() {
	...
	err := m.executeStruct.validateTestOutput(expectedTl, fruits.Apple)
	m.EqualError(err, "the following pkgs are not valid: [pkg=go-shift-register/cmd/pkg-test cov=79.000000 under the 80.000000% minimum line coverage]")
}
